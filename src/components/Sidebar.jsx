  ...
  Support: [
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'Details', path: '/inbox-details', id: 'inboxDetails', pro: true }
      ]
    }
  ],
  Others: [
...
const Sidebar = ({ sidebarToggle = false }) => {
      ...
      <li>
        ...
        >
          ...
          {hasDropdown && (
            <FontAwesomeIcon
              icon={faChevronDown}
              className={`menu-item-arrow ${isActive ? 'menu-item-arrow-active' : 'menu-item-arrow-inactive'} ${sidebarToggle ? 'lg:hidden' : ''}`}
            />
          )}
        </Link>

        {hasDropdown && (
          <div className={`translate transform overflow-hidden ${isActive ? 'block' : 'hidden'}`}>
                ...
                <li key={idx}>
                  <Link
                    ...
                    className={`menu-dropdown-item group ${
                      page === subItem.id ? 'menu-dropdown-item-active' : 'menu-dropdown-item-inactive'
                    }`}
                  >
                    {subItem.name}
  ...
  return (
    <aside className={`sidebar fixed top-0 left-0 z-9999 flex h-screen w-[290px] flex-col overflow-y-auto 
      ...
      <div className={`sidebar-header flex items-center gap-2 pt-8 pb-7 ${sidebarToggle ? 'justify-center' : 'justify-between'}`}>
        <Link to="/">
          ...
          <img className={`logo-icon ${sidebarToggle ? 'lg:block' : 'hidden'}`} 
      ...
      <div className={`mx-auto mb-10 w-full max-w-60 rounded-2xl bg-gray-50 px-4 py-5 text-center 
        ...
        <a href="https://tailadmin.com/pricing" 
           target="_blank" 
           rel="nofollow"
